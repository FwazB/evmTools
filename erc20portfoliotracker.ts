...
class PortfolioTracker {
  ...
  /**
   * Aggregate portfolio across all wallets
   */
  ...
  /**
   * Display portfolio in formatted console output
   */
  ...
  getPortfolioStats(): {
    totalWallets: number;
    totalAssets: number;
    largestHolding: AssetValue | null;
    diversificationScore: number;
  } {
    const totalWallets = this.wallets.length;
    const totalAssets = Object.keys(this.portfolioData.aggregatedBalances).length;
    
    // This would need the calculated values - simplified for example
    return {
      totalWallets,
      totalAssets,
      largestHolding: null, // Would be calculated from valueBreakdown
      diversificationScore: totalAssets > 0 ? Math.min(totalAssets / 10, 1) : 0
    };
  }
